...
export default function Exams() {
  ...
  const save = () => {
    ...
    if (modal === 'add') {
      ...
      setData(data.map((e) => (e.id === sel.id ? { ...sel, ...form } : e)))
  ...
  const classOptions = useMemo(() => {
    return (classes || [])
      .map((c) => {
        const label = c?.className && c?.division ? `${c.className}-${c.division}` : c?.className
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{data.length} exams scheduled</p>
        </div>
        <button
          onClick={() => {
            setForm(BLANK)
      ...
      <div className="card overflow-hidden p-0">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
            <thead>
              <tr>
                {['Exam Name', 'Type', 'Class', 'Subject', 'Date', 'Max Marks', 'Status', 'Actions'].map((h) => (
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
            <thead>
              <tr>
                {['Student', 'Class', 'Exam', 'Subject', 'Marks', 'Grade', 'Remarks'].map((h) => (
                  <th key={h} className="th">
                    {h}
                  </th>
                ))}
      ...
      >
        {classesError ? (
          <div className="mb-4">
            <EmptyState icon={null} title="Failed to load classes" desc={classesError} />
          </div>
        ) : null}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField label="Type">
            <select className="input" value={form.type} onChange={(e) => f('type', e.target.value)}>
              {['Unit Test', 'Mid Term', 'Term Exam', 'Practical', 'Final'].map((t) => (
          ...
          <FormField label="Class" required>
            <select
              className="input"
              value={form.class}
              onChange={(e) => f('class', e.target.value)}
              disabled={classesLoading || !!classesError}
            >
              <option value="">{classesLoading ? 'Loading classes...' : 'Select'}</option>
              {classOptions.map((c) => (
          ...
          <FormField label="Duration">
            <input className="input" value={form.duration} onChange={(e) => f('duration', e.target.value)} />
          </FormField>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          setData(data.filter((e) => e.id !== confirm))
          setConfirm(null)
